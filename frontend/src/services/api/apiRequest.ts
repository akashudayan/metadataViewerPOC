...
export default class ApiRequest {
  ...
  successReturn = async (arg0: BaseResponse): Promise<any> => await Promise.resolve(arg0)

  constructor(url: string, customErrorCheck?: (arg0: BaseResponse) => any | undefined) {
    this.baseUrl = url
  ...
  async get(): Promise<any> {
    ...
    return await fetch(this.baseUrl + ApiRequest.jsonToQueryString(this.queryParams), {
      method: 'GET',
      headers: this.header
    })
   ...
   * @returns
   */
  static jsonToQueryString(json: Record<string, string | number | boolean> | undefined): string {
    if (json === undefined) {
      return ''
    }
