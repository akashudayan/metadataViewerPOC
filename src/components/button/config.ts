...
export type ButtonIntent = Extract<Intent, 'neutral' | 'primary' | 'success' | 'warning' | 'danger'>

export const VARIANT_MAP = {
  solid: Object.entries(INTENT_MAP).reduce<Record<keyof typeof INTENT_MAP, CSSObject> | any>(
    (acc, [key, intent]) => {
  ...
  outline: Object.entries(INTENT_MAP).reduce<Record<keyof typeof INTENT_MAP, CSSObject> | any>(
    (acc, [key, intent]) => {
      acc[key as ButtonIntent] = {
  ...
  ghost: Object.entries(INTENT_MAP).reduce<Record<keyof typeof INTENT_MAP, CSSObject> | any>(
    (acc, [key, intent]) => {
      acc[key as ButtonIntent] = {
        border: `1px solid ${intent.fg}`,
        backgroundColor: 'transparent',
